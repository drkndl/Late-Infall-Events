...
def main():
    ...
    folder = Path("F:/cloud_disk_it450_rotX45/")                    # Folder with the output files
    ...
    fig.tight_layout()
    plt.savefig(f'{fig_imgs}/logMdot_vs_t.pdf', bbox_inches="tight", dpi=300)
    plt.show()
    ...
    for zmax in zmax_array:
        ...
        zmax_labels[zmax] = fr"{int(zmax/Hc)}H ({theta_sel_max}$\degree$)"
    ...
    fig.tight_layout()
    ax.legend(loc="lower left")   # loc='upper left', 
    plt.savefig(f'{fig_imgs}/logMdot_vs_t_all_zmax.pdf', dpi=300, bbox_inches="tight")
    plt.show()
