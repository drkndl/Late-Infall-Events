...
def main():
    ...
    for i in range(0, it+1, N):
        ...
        rho = get_data(folder, "dens", i, domains)         # Load 3D array of density values
        vphi = get_data(folder, "vx", i, domains)          # Load 3D array of azimuthal velocities v_phi
        vrad = get_data(folder, "vy", i, domains)          # Load 3D array of radial velocities v_rad
        vthe = get_data(folder, "vz", i, domains)          # Load 3D array of colatitude velocities v_theta
        energy = get_data(folder, "energy", i, domains)    # Load 3D array of energies 
        ...
        write_dat(ext_folder, "dens", i, rho_ext)       # Write 3D array of densities into dat file (nth, nr, nphi+2)
        write_dat(ext_folder, "vx", i, vphi_ext)        # Write 3D array of vx into dat file (nth, nr, nphi+2)
        write_dat(ext_folder, "vy", i, vrad_ext)        # Write 3D array of vy into dat file (nth, nr, nphi+2)
        write_dat(ext_folder, "vz", i, vthe_ext)        # Write 3D array of vz into dat file (nth, nr, nphi+2)
        write_dat(ext_folder, "energy", i, E_ext)       # Write 3D array of energy into dat file (nth, nr, nphi+2)
